...
def parameter_search(ntrials, objective_function, task):
    ...
    search_space = {'num_dense_layers': hp.choice('nlayers', [2]),
                    'num_dense_units': hp.choice('num_dense', [500]),
                    'num_epochs': hp.choice('num_epochs',  [100]),
                    'num_lstm_units': hp.choice('num_lstm_units', [200]),
                    'num_lstm_layers': hp.choice('num_lstm_layers', [2]),
                    'learn_rate': hp.choice('learn_rate', [3e-4]),
                    'mb_size': hp.choice('mb_size', [32]),
                    'l2reg': hp.choice('l2reg', [0.0]),
...
##                    }
    ...
    f = open('trials_'+task+'.txt', "wb")
    pickle.dump(trials, f)
    f.close()
    
    filename = 'bestparams_'+task+'.txt'
    f = open(filename, "wb")
    pickle.dump(bp, f)
    f.close()
